...
flow-v2 使用 Experiment schema v1：`benchmark_measurements.comparisons` 逐 Case 提交同一次实验的原始 `reference`/`candidate` 数值。flow-v3 使用 Experiment schema v2：开始前必须核对 `experiment_cycle` 与未消费的 `reference_receipt`，`reference_receipt_id` 必须原样绑定本 Work；本 Agent 只能提交 Candidate 的 `benchmark_measurements.cases[].values`，不得重测或自报 reference。daemon 会把 Receipt 的 reference 与 Candidate 原始值连接，按冻结 Case、weight、direction 与 `iteration_performance_gate` 重算。`kept` 必须给出 `checkpoint_sha`、`correctness.benchmark_integrity` 与完整 Candidate measurements；`rejected`/`inconclusive` 禁止给 `checkpoint_sha`，可提交完整 Candidate measurements，部分测量只能作为 artifact。超过 10x 时按 schema 提供真实独立 retest。

flow-v3 每个 Iteration Work 最多记录一个 Experiment，Reference Receipt 也只能消费一次；record 成功后不得再 commit 或记录第二个 Experiment。若继续同一 Attempt，调用终态 `start_next_experiment`，Pika 会先安排新的 Benchmark Work，再创建全新的 Iteration Work；若结束 Attempt，则调用 `finish_iteration`。没有记录 Experiment 而放弃 Receipt 时，`start_next_experiment.reason` 必须说明原因。
