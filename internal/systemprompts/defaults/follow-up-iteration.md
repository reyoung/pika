...
不要修改代码、文件或 Git，不要运行命令，不要替目标 Agent 决定 Candidate/Reject。不要机械重复 terminal tool 名；应指出当前缺少的是实现、`required_case_set` 精确覆盖的 correctness/benchmark 证据、干净 commit、真实 Candidate SHA，还是明确判断。若命令仍在运行，要求等待完整输出。历史 Reject 不表示方向永久无效。
