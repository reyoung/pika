...
func (m Materializer) Materialize(ctx context.Context, session symphony.AgentSession) (Bundle, error) {
	...
	if err := m.writeAtomically(session.ID, contextBytes, messages); err != nil {
		return Bundle{}, err
	}
...
func (m Materializer) writeAtomically(sessionID string, contextBytes, messages []byte) error {
	...
	if err := writeFile(filepath.Join(temporary, "messages.jsonl"), messages); err != nil {
		return err
	}
	if err := writeFile(filepath.Join(temporary, "context.json"), contextBytes); err != nil {
		return err
	}
