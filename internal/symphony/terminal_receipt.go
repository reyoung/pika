...
func (e *Engine) WorkRoleTerminalReceipt(ctx context.Context, workID string, role WorkRole) (Receipt, bool, error) {
	...
	commandTypes, eventTypes, payloadKey, correlationKey, ok := roleTerminalEvents(role)
	if !ok {
	...
	rows, err := e.db.QueryContext(ctx, `SELECT r.request_id, r.receipt_id, r.command_type, r.revision, r.result_json, e.event_type, e.payload_json
		...
		ORDER BY r.created_at DESC`, optimizationID)
	if err != nil {
	...
	for rows.Next() {
		...
		if !commandTypes[receipt.Command] || !eventTypes[eventType] {
			continue
		}
...
func roleTerminalEvents(role WorkRole) (commandTypes, eventTypes map[string]bool, payloadKey, correlationKey string, ok bool) {
	switch role {
	case RoleBaselineDraft:
		return map[string]bool{"submit_baseline_definition": true}, map[string]bool{"baseline.submitted": true, "baseline.submitted_while_draining": true}, "draft_work_id", "baseline_revision_id", true
	case RoleBaselineVerification:
		return map[string]bool{"finish_baseline_verification": true}, map[string]bool{"baseline.verification_finished": true}, "verification_work_id", "baseline_revision_id", true
	case RoleDiagnosis:
		return map[string]bool{"finish_diagnosis": true}, map[string]bool{"diagnosis.finished": true}, "work_id", "diagnosis_id", true
	case RoleBenchmark:
		return map[string]bool{"finish_iteration_benchmark": true}, map[string]bool{"experiment_cycle.reference_measured": true, "experiment_cycle.benchmark_unavailable": true}, "benchmark_work_id", "experiment_cycle_id", true
	case RoleIteration:
		return map[string]bool{"finish_iteration": true, "start_next_experiment": true}, map[string]bool{"iteration.finished": true, "experiment_cycle.next_started": true}, "work_id", "attempt_id", true
	case RoleIntegration:
		return map[string]bool{"finish_integration": true}, map[string]bool{"integration.finished": true}, "integration_id", "integration_id", true
	case RoleFollowUp:
		return map[string]bool{"submit_followup_message": true}, map[string]bool{"followup.message_submitted": true}, "generator_work_id", "followup_request_id", true
	default:
