...
New Baseline Revisions freeze a `benchmark_measurements` version 1 contract: Case weights plus metric identity, label, unit, role, direction, sample statistic, and aggregation. Exactly one metric is primary. Terminal MCP transactions submit per-Case values; Pika computes ratios, regressions, aggregates, maximum Case speedup, and the 10x gate inside the transaction before persisting normalized rows.
