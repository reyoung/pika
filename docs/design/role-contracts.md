...
Baseline Verification establishes Development Baseline measurements and proves that the frozen protocol can judge future Candidates. Candidate improvement thresholds and stop conditions do not apply to the Development Baseline itself; a zero improvement relative to itself is expected. The Iteration performance gate and the overall performance goal have different scopes: the former is the lowest reliable, noise-derived incremental improvement from a Round's reference checkpoint, while the latter describes cumulative progress from the Development Baseline and when Optimization should stop. Verification rejects a Definition that copies the cumulative goal into the per-Round gate or would exclude otherwise valid improvements above measurement noise. The Baseline is rejected when the Development Baseline is incorrect, unmeasurable, incomplete, unstable or implausible, or when the declared gate cannot be computed or justified from the evidence.
...
The Role may modify only its current Round's assigned, independent Git worktree. Every Round has a distinct branch, path, Work, and Agent cwd; previous Round worktrees are never reused as the current code workspace. It cannot update Best. It reads current Baseline, Best, Back-off message, and stale-round recovery facts. For Round 2+, `previous_round` points to the immediately preceding Round's complete cross-Session normalized history and must be read as fully as practical before changing code. Separately, Pika selects the newest N accepted/rejected Attempts without outcome rebalancing, excludes current/non-terminal/cancelled Attempts, then presents the selection chronologically. These terminal histories are an index: each entry has a durable summary and digest-addressed detail files, but the Agent reads details only when relevant rather than loading every file. An empty Journal remains a valid zero-record file. A rejection proves only that execution failed under its recorded conditions and does not permanently ban the direction.
