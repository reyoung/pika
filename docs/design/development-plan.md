...
| Scheduler Pause/Resume | Implemented; all dedicated gates pass | Orthogonal persistent Scheduler state, audited per-Session control cycles, priority outbox gating, frozen Follow-up clocks, protocol v2, `pause`/`resume`, and the Workspace `open` rename pass race, real-Herdr/fake-Agent, and credentialed Codex/Cursor validation. |
...
Local verification recorded on 2026-08-27, 2026-08-28, and 2026-09-01:

- Scheduler Pause/Resume verification on 2026-08-28 passed `make verify`, the complete `make herdr-integration` matrix, and every constituent of the opt-in `make real-pause-resume-integration` gate. The real Herdr/fake-Agent path observed a `working` Session, delivered provider-owned `ctrl+c`, retained the same Pika Session and pane, then delivered the exact `继续` prompt before scheduling was released. Pause/resume is also embedded in each complete credentialed workflow after its first real provider Tool event: Codex passed in 435.81 seconds with 11 Sessions/64 Tool events; all-Cursor passed in 360.92 seconds with 10 Sessions/66 Tool events; Cursor/Codex/Cursor/Codex/Cursor passed in 402.41 seconds with 10 Sessions/80 Tool events; and Codex/Cursor/Codex/Cursor/Codex passed in 381.13 seconds with 12 Sessions/57 Tool events. Every workflow continued through Follow-up, fresh-Session crash recovery, sibling-isolated cancellation, Integration Back-off, Best advancement, and graceful drain after Scheduler resume.
...
- `make real-codex-integration` passed the complete disposable real-Codex release matrix in 519.56 seconds. A dedicated Follow-up Session delivered guidance to a still-running Verification Work; a daemon process crash replaced that target with a fresh Pika Session and a distinct provider session; Baseline Revision 1 was accepted; two Iteration Sessions ran concurrently; cancelling one Attempt left its sibling pending; the first Integration backed off to a new Round with its message preserved; and a later Candidate advanced `pika/best` from sequence 0 to 1. The run drained normally with 10 distinct provider sessions and 61 observable tool events retained. Baseline Draft and Iteration committed through grant-scoped `commit_changes` while coding Agents remained in `workspace-write`.
...
- `make real-cursor-integration` passed against Cursor `2026.08.31-4057e58` in 339.33 seconds. The all-Cursor Optimization delivered Follow-up to a pending Verification Work, replaced that Work with a fresh provider Session after daemon crash, isolated cancellation between parallel Iterations, backed off Integration, advanced Best to sequence 1, and drained normally with 11 provider Sessions and 111 observable Tool events.
- `make real-mixed-provider-integration` passed both alternating assignments in 1102.36 seconds. Cursor/Codex/Cursor/Codex/Cursor completed in 672.12 seconds with 13 provider Sessions and 104 observable Tool events; Codex/Cursor/Codex/Cursor/Codex completed in 430.24 seconds with 11 provider Sessions and 72 observable Tool events. Together they prove both cross-provider Follow-up directions, configured-provider fresh recovery, cancellation isolation, Back-off across providers, Best advancement, and graceful drain without fallback.
...
| Scheduler Controller | `Apply(PauseScheduler|ResumeScheduler) -> {Receipt, ControlCycle}` | transition/runtime serialization, exact priority effect dispatch, bounded parallel Session control, replay/no-op behavior | Scheduler Pause/Resume |
...
Cursor support is pinned to the real-gate-tested CLI version `2026.08.31-4057e58`. A mismatch is a hard preflight failure, not a warning or silent capability downgrade.
...
The wrapper launches the interactive TUI with validated user args followed by `--workspace <repository> --model <parameterized-model>` and, for automatically kicked-off Work, a positional `<initial-prompt>`. It defaults to `--yolo` unless the Role args explicitly select `--sandbox enabled`. Supplying automatic kickoff positionally avoids a startup `agent.prompt` race. The first Baseline Draft omits it, waits for `interactive_ready`, and lets the operator author the first User Turn. The wrapper never uses `--print`, `--resume`, or `--continue`. When the positional prompt is present, Pika accepts Herdr's successful `agent.start` response without synchronously waiting for idle because the prompt may already be executing. Its grant is valid while the Session is `starting` or `running`, and launch binding promotes it to `running`. Later Follow-up and human steering still use Herdr `agent.prompt`. Cleanup removes only private Session state after the child exits, and startup removes stale Session directories not referenced by an active Session.
...
All five Phase 9 build slices and both credentialed real-provider gates pass. Cursor `2026.08.31-4057e58` is release-supported alongside Codex, including the two accepted static mixed-role assignments described above.
