...
Pika 后端使用 Elixir/OTP，把长期运行的 Campaign、Backend Session、归并队列和主线复验映射为受监督进程，以利用进程隔离、消息传递和监督树处理多 Agent 并发与局部失败。Agent 协议位于 `Pika.AgentBackend` Behaviour 后；Codex 直接实现 App Server stdio JSON-RPC，Cursor 使用经过 conformance 的 Elixir ACP 库或受控 fork，而不为此改回 TypeScript 后端。
