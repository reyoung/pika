...
Campaign Spec 确认全量 Case 集以及用户指定的正式 Pair 数和有效 Pair 门槛，初始 Baseline 对全部 Case/Metric 执行该正式测量协议。Baseline Agent 随后自动选择最多十个 Case 形成首个迭代采样集；日常 Attempt 只需对其启动时的 Sampling Revision 做正式测量。
