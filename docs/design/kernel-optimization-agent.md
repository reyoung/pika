...
- 一个调优任务允许多个候选尝试并行执行。
- 用户只配置 Iteration Agent 并发度；Pika 不暴露全局并发度或 Boundary、Plan、主线复验等其他角色的并发参数。
...
- 是否启动主线 Agent 对合入结果进行复验可由用户配置，默认关闭。主线复验最多一个并发，并且发生在 Iteration 开发 Agent Merge 之后。
- 主线复验不阻塞归并队列；后续 Iteration 开发 Agent 可以继续 Merge。
- 如果主线复验失败，主线 Agent 在 Campaign Best Branch 最新历史上创建 revert commit，并修正已记录的 Metric；不能使用 rebase、reset 或其他改写历史的方式撤销。
- 每个主线复验任务固定测试对应候选的 squash commit SHA，并在独立 worktree 中执行；不能测试可能已包含后续 Merge 的动态分支 HEAD。
- 需要撤销时，在 Campaign Best Branch 最新 HEAD 上对被复验的 squash commit 执行 revert。
- 复验失败立即产生 `RevertRequired` 事件，注入所有正在运行以及之后启动的 Agent。主线 Agent 获取唯一归并锁后执行 revert、解决冲突并验证结果。
- 如果在配置的尝试预算内仍不能安全完成 revert，调优任务进入 `Blocked`：已有 Iteration 可以保存工作，但禁止继续 Merge。
- Revert 成功后，排队候选仍必须在包含该 revert 的最新 HEAD 上重放并重新测试。
- 后续 Iteration 开发 Agent 在规划、重放和归并时必须获知并处理相关 revert。
- 如果主线复验与 Iteration 开发 Agent 报告的 Metric 相差超过 1%，直接用最新结果替换该版本的结构化 Metric，不保留两套结构化观测。
- Pika 不为此建立 GPU Worker、远程执行协议或节点资源模型。

## 已确认的性能判定原则

- 接受条件必须容忍 Benchmark 测量噪声，而不是按聚合值要求严格零退化。
- 每个 Benchmark Case 由 shape、dtype、layout、输入分布和可选线上频率权重定义；每个 `(Case, Metric)` 独立参与接受判断。
- 至少一个目标 Case 的目标 Metric 必须真实改善；所有保护 Case 的保护 Metric 都不得退化超过噪声容忍值。
- 线上频率权重只用于综合评分、候选排序和 UI，不能用高频 Case 的收益抵消保护 Case 的退化。
- 观察 Case 只记录和展示，不参与接受判断。
- 正式性能判定必须在 warmup 后交错执行 `baseline → candidate` 的多轮配对测量，使用配对比值的中位数和 MAD 自动估算每个 Metric 的噪声容忍值。
- 每个 Case 默认 warmup 10 次并执行 30 个 Pair，次序交替为 `baseline → candidate` 与 `candidate → baseline`；Campaign 可以覆盖样本数。
...
- `max_attempts` 在创建候选尝试时计数；Accepted 和 Rejected 都计入，Plan、恢复会话、主线复验和 Revert 不计入。
- 达到尝试预算或性能目标后停止创建新候选，但已经运行或进入归并队列的候选继续完成。
- 多个性能目标默认必须全部达到；Campaign 可以显式配置为任一目标达到即停止。
- 默认不因连续若干次无提升而提前停止；只有用户显式配置时才启用 Plateau 条件。
- 启用主线复验时，所有复验和 Revert 工作排空后，Campaign 才能进入 `Completed`。
...
- SQLite 使用当前状态表加 `domain_events` 的混合模型。主要表包括 `campaign`、`spec_revisions`、`benchmark_cases`、`metric_definitions`、`best_revisions`、`attempts`、`attempt_metrics`、`agent_sessions`、`agent_messages`、`guidance`、`artifacts`、`sync_runs`、`operation_intents` 和 `domain_events`。
- 每个 Attempt/Case/Metric 只保留最新结构化 Metric；主线复验覆盖旧值。`domain_events` 保存状态转换和 UI 时间线，但不复制被覆盖的旧 Metric 快照。
...
- Boundary、Plan、Iteration、主线复验和旁路对话角色都可以选择命名 Agent Profile。
...
- 每个点使用该 Attempt 当前最新 Metric；主线复验修正后直接更新对应点。
