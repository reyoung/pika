...
The domain transaction also enforces `benchmark_integrity` schema version 1. A Baseline Definition freezes the exact Full Case Set, formal repeat and invocation counts, device-resident protected inputs and Oracle outputs, per-invocation restore/check protocol, compact deferred host transfer, and kernel/end-to-end timing boundaries. Accepted Baseline Verification evidence and Integration validation must cover exactly that Full Case Set and prove `input_restores == checked_invocations == repeats * (warmups + measured)` with no mismatch, nonfinite value, or canonical-input mutation. Candidate Iteration evidence instead covers exactly its Round-frozen Iteration Case Snapshot. Integration validation additionally declares primary and maximum per-Case speedup. Either value at or above 10x requires a passing independent retest with changed canonical inputs, output sentinel, cold-start/setup/steady-state/end-to-end reporting, and a fair timing boundary before Pika will create a Git Intent.

Rejected `finish_integration` may carry severity-ranked `regression_cases`. Every item is validated against the Full Case Set and must classify a case-specific frozen-gate violation as `correctness` or `performance`; environment failures and noise are not Regression Cases. The daemon validates the entire report, skips Cases already selected, and atomically appends at most the first three unseen Cases. Existing Round snapshots never change, no rejection is retried automatically, and the Iteration Case Set never shrinks. Accepted or stale outcomes cannot report Regression Cases.
...
The instance wrapper adds the Session-frozen Pika System Prompt as a per-launch `developer_instructions` override and launches Codex with `--yolo` by default. This is the default unattended Agent sandbox policy and enables external network/H20 tooling; a future per-Role sandbox policy may replace it. It also passes `--dangerously-bypass-hook-trust`, ensuring Pika and Herdr lifecycle hooks are active before the first Turn instead of leaving the pane behind a trust dialog. Git metadata commits and Best application still use scoped MCP control-plane tools. Matching hooks from the base config and profile are additive. The profile name is reserved; Role Agent configuration cannot supply a second `--profile` argument.

The bypass is an invocation-only property of the Pika wrapper; it does not modify the user's base Codex configuration. Because Codex loads matching hooks additively, the operator is responsible for ensuring every enabled base-profile hook is trusted. Persisted Pika hook hashes are retained only when valid and keyed to the exact managed hook and profile path.
