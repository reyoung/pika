...
4. Atomically materialize `contexts/<session-id>/context.json` and `messages.jsonl` before provider launch.
5. Record both relative paths, SHA-256 digests, byte sizes, record count, and schema version in `context_snapshots`.

The full JSON Schemas for the context document and each JSONL record are versioned with the producer and embedded verbatim in every consuming System Prompt. A dispatch retry verifies and reuses the frozen files; a new or recovery Agent Session creates a new reviewable snapshot.
