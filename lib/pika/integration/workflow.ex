defmodule Pika.Integration.Workflow do
    ...
    transaction =
      Repo.transaction(fn ->
        ...
        append_event(
          ...
          %{best_sha: details["best_after_sha"], best_revision: sequence},
          now
        )
  ...
  defp append_sampling_feedback(attempt, feedback, now) do
    current_ids = sampling_case_ids(attempt.sampling_revision_id)
    ...
    if additions == [] do
      ...
      [[baseline_revision_id, current_sequence]] =
        Repo.query!(
          "SELECT baseline_revision_id, sequence FROM sampling_revisions WHERE id = ?",
          [attempt.sampling_revision_id]
        ).rows
  ...
  defp integration_run(attempt_id) do
    case Repo.query!(
           ...
           SELECT id, status, expected_best_sha, validation_sha256, statistics_json,
                  benchmark_artifact_id, outcome
           FROM integration_runs WHERE attempt_id = ?
           """,
           [attempt_id]
         ).rows do
      [[id, status, expected, validation_sha, statistics, benchmark_artifact_id, outcome]] ->
        intent_id =
          case Repo.query!(
