defmodule Pika.Phase2.Store do
  @moduledoc false
  ...
  @durable_fields ~w(
    ...
    baseline baseline_retry_count baseline_error sampling_revisions iteration_sampling phase_kickoffs
    pending_confirmation_input last_error backend_phase required
  )a
  ...
  def persist(state) do
    ...
    error -> {:error, {:phase2_persist_failed, Exception.message(error)}}
  end
  ...
  defp persist_spec!(campaign_id, state, now) do
    ...
    Repo.query!(
      ...
      [
        ...
        baseline_sha,
        Jason.encode!(Pika.Phase0.Evidence.json_safe(ref_snapshot)),
        Jason.encode!(Pika.Phase0.Evidence.json_safe(skill_snapshot)),
        confirmed_at,
        now,
        now
      ]
