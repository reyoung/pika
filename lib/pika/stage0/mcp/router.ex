defmodule Pika.Stage0.MCP.Router do
  @moduledoc false
  ...
  defp dispatch_rpc(conn, token, %{"method" => "initialize", "id" => id}) do
    case Pika.Stage0.Campaign.authorize(token) do
      :ok ->
        rpc_result(conn, id, %{
          "protocolVersion" => "2025-06-18",
          "capabilities" => %{"tools" => %{"listChanged" => false}},
          "serverInfo" => %{"name" => "pika-stage0", "version" => "0.1.0"}
        })
  ...
  defp dispatch_rpc(conn, token, %{
    ...
    case Pika.Stage0.Campaign.mcp_call(token, name, arguments) do
  ...
  defp tools do
    [
      tool(
        "get_context",
        "Read the current Stage0 Campaign state and required Boundary operations.",
        %{}
      ),
      tool(
        "register_artifact",
        "Register an existing file under the Stage0 Workspace.",
        %{
      ...
      tool(
        ...
        %{
          "idempotency_key" => string(),
          "spec" => Pika.Stage0.Spec.json_schema()
        },
  ...
  defp tool_result(conn, id, value) do
    value = Pika.Phase0.Evidence.json_safe(value)
  ...
  defp tool_error(conn, id, code, message, details) do
    value = %{
      "code" => to_string(code),
      "message" => message,
      "details" => Pika.Phase0.Evidence.json_safe(details)
    }
