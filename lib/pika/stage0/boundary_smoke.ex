defmodule Pika.Stage0.BoundarySmoke do
  @moduledoc false

  alias Pika.Stage0.{Campaign, Git, Workspace}
  ...
  def run(backend, opts \\ []) when backend in [:codex_app_server, :cursor_acp] do
    ...
    try do
      ...
      Pika.Phase0.Evidence.write_json(
  ...
  defp create_source_repo do
    repo =
      Path.join(System.tmp_dir!(), "pika-stage0-real-smoke-#{System.unique_integer([:positive])}")
  ...
  defp do_assert_eventually!(fun, deadline) do
    cond do
      ...
      System.monotonic_time(:millisecond) >= deadline ->
        raise "Stage0 Boundary smoke timed out: #{inspect(Campaign.snapshot())}"
