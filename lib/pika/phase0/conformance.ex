defmodule Pika.Phase0.Conformance do
  @moduledoc "Real Codex App Server and Cursor ACP Phase 0 conformance runner."

  alias Pika.AgentBackend
  alias Pika.AgentBackend.{EventCollector, JSONLWriter, Replay}
  alias Pika.Phase0.{Evidence, Skill}
  ...
  def run(opts \\ []) do
    ...
    artifact_dir = opts |> Keyword.get(:artifact_dir, "artifacts/phase-0") |> Path.expand()
  ...
  defp run_backend(backend, module, workspace, artifact_dir, mcp_url, skill, control_probes) do
    ...
    isolation =
      run_isolation_probe(backend, module, handle, workspace, artifact_dir, mcp_url, skill.path)
  ...
  defp assert_no_resume!(backend, records) do
    forbidden = if backend == :codex_app_server, do: "thread/resume", else: "session/load"

    if Enum.any?(records, &(get_in(&1, ["payload", "method"]) == forbidden)) do
