defmodule Pika.Agent.BaselineAlignmentRolePromptTest do
  ...
  test "returns the complete prompt without a context section", %{schemas: schemas} do
    ...
    assert prompt =~ "必须至少成功调用一次 `ask_questions`"
    assert prompt =~ "唯一允许的用户决策通道"
    assert prompt =~ "禁止在普通文本中列出问题或选项"
    ...
    assert prompt =~ "Benchmark 单进程强制协议"
    assert prompt =~ "同一个长期运行的 Python 进程或同一次 `torchrun`"
    assert prompt =~ "禁止在 Shell 或其他调度层按 Case 循环"
    assert prompt =~ "只初始化一次 CUDA / Distributed / NCCL"
    assert prompt =~ "所有 Case 仍必须由一个 Python / `torchrun` invocation 完成"
    assert prompt =~ "`guard`"
