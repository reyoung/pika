...
Code.require_file(Path.join(project_root, "priv/v2/roles/baseline_verify_followup.ex"))

defmodule Pika.Agent.BaselineVerifyFollowupRolePromptTest do
  use ExUnit.Case, async: true

  alias Pika.Agent.RolePrompt.FollowupInput
  alias Pika.Agent.RolePrompts.BaselineVerifyFollowup

  @project_root Path.expand("../../..", __DIR__)
  @fixture_dir Path.join(@project_root, "test/fixtures/v2/roles/baseline_verify_followup")

  test "returns the complete prompt without a title" do
    input = input()
    result = BaselineVerifyFollowup.system_prompt(input)

    assert {:ok, prompt} = result

    if System.get_env("SHOW_V2_BASELINE_VERIFY_FOLLOWUP_PROMPT") == "1" do
      IO.puts("\nCALL_RESULT=#{inspect(elem(result, 0))}\n")
      IO.puts(prompt)
    end

    assert String.starts_with?(prompt, "你是 Pika 的 Baseline Verify Follow-up Agent。")
    refute String.starts_with?(prompt, "#")
    refute prompt =~ "# Baseline Verify Follow-up System Prompt"
  ...
  test "reports every missing injected file" do
    ...
    assert {:error, {:missing_prompt_files, missing}} =
             BaselineVerifyFollowup.system_prompt(input)
