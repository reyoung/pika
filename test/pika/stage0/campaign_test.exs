defmodule Pika.Stage0.CampaignTest do
  use ExUnit.Case, async: false

  alias Pika.Stage0.{ArtifactStore, Campaign, Git, Workspace}
  ...
  test "runs Alignment, explicit confirmation, setup merge and Baseline to Optimizing", %{
    ...
    [samples, correctness, profiler] =
      Stage0Fixtures.write_baseline_artifacts(workspace, best_sha, skill.sha)
  ...
  test "same idempotency key is replayed and conflicting request is rejected" do
    request = %{"idempotency_key" => "spec-same", "spec" => Stage0Fixtures.spec()}
  ...
  test "allows one complete Baseline rerun and blocks the second insufficient result", %{
    ...
    [samples, correctness, profiler] =
      Stage0Fixtures.write_baseline_artifacts(workspace, best_sha, skill.sha, 23)
