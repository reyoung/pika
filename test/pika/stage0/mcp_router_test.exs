defmodule Pika.Stage0.MCP.RouterTest do
  ...
  test "authenticates initialize and exposes the Boundary tool contract" do
    assert %{"result" => %{"serverInfo" => %{"name" => "pika-stage0"}}} =
             rpc(@token, %{
               "jsonrpc" => "2.0",
               "id" => 1,
               "method" => "initialize",
  ...
  test "rejects a missing bearer token" do
    conn =
      ...
      |> Pika.Stage0.MCP.Router.call(Pika.Stage0.MCP.Router.init([]))
