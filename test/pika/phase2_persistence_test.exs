defmodule Pika.Phase2PersistenceTest do
  use ExUnit.Case, async: false

  alias Pika.Phase2.{Metrics, Registry, Store}
  ...
  @token "phase2-persistence-token"
  ...
  setup do
    ...
    assert {:ok, stage_workspace} = Pika.Phase2.Workspace.prepare(workspace, campaign)
  ...
  test "persists and restores Alignment through Baseline as normalized Phase 2 state", context do
    pid = start_campaign(context)
    harness_args = Stage0Fixtures.create_harness(context.stage_workspace.setup_worktree)
    ...
    [samples, correctness, profiler] =
      Stage0Fixtures.write_baseline_artifacts(
